...
async function setMAXIVColourTheme() {
  ...
  await db.updateSettings({
    ...
    settingsValue: 'maxiv.svg',
  });
}

async function enableDefaultMAXIVFeatures() {
  const db = container.resolve<AdminDataSource>(Tokens.AdminDataSource);
...
export async function configureMAXIVDevelopmentEnvironment() {
  await setMAXIVColourTheme();
  await enableDefaultMAXIVFeatures();
  await setTimezone();
